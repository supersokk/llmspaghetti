...
def write_litellm_config(form_data):
    ...
    # With no cloud keys there are no model_list entries; an empty `model_list:`
    # parses as null and the router's alias loader chokes — emit `[]` instead.
    has_cloud = any(form_data.get(k) for k in
                    ("anthropic_key", "openai_key", "groq_key", "cerebras_key"))
    lines = [
        "# LiteLLM Proxy — generated by LLMSpaghetti first-boot wizard",
        "# Edit with: spag config",
        "",
        "model_list:" if has_cloud else "model_list: []",
        "",
        "  # Local models are routed straight to Ollama by their raw name (the router",
        "  # forwards bare Ollama names directly) — no LiteLLM alias needed. Only cloud",
...
def write_router_roles_config(form_data):
    ...
    # The local model the user picked — used by its RAW downloaded name (no alias).
    ...
    roles = {
        ...
        "document":  "claude-sonnet" if has_anthropic else local_model,
        "general":   local_model,
    ...
    lines = [
        ...
        "# Local roles use the RAW Ollama model name (routed straight to Ollama).",
        "# Cloud roles use a model_name from litellm_config.yaml.",
        "",
        # Fallback when a role is unset — a real model name, never an alias.
...
async def apply_setup(
    ...
    form_data = {
        ...
        "groq_key":      groq_key,
        # The first model the user picked becomes the default local model (used
        # by its raw name). Falls back to a tiny CPU-friendly model if none chosen.
    ...
    # Pull models BEFORE starting the stack so the default local model exists when
    # the router first tries to reach it. Record the selection so /api/status can
