...
- ✅ **Speed up model downloads** — Image-tab downloads (engine catalog + HF picker)
...
- ✅ **HuggingFace login (gated/private models)** — `HF_TOKEN` field in Cockpit
  ...
  repos, unlocks gated ones). A 401/403 surfaces a clear "this model looks gated —
  add your HF token in Settings + accept the licence" message.
